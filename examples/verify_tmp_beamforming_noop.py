#!/usr/bin/env python3
"""Prepare one separately authorized TMP beamforming current-value no-op."""
...
def _required(value: str, name: str) -> str:
    if value:
        return value
    raise ValueError(f"Failed to run TMP beamforming no-op verification: {name} is required")
...
def _progress(event: str) -> None:
    labels = {
        "preflight": "[1/3] Reading current beamforming state...",
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.timeout <= 0:
        raise ValueError(
            "Failed to run TMP beamforming no-op verification: --timeout must be positive"
        )
    if args.confirm != TMP_BEAMFORMING_NOOP_CONFIRMATION:
    ...
    if not result.verified_noop:
        raise RuntimeError(
            f"Failed to verify TMP beamforming no-op: result status is {result.status}"
        )
