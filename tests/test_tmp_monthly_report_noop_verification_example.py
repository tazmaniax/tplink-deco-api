"""Tests for the prepared TMP monthly-report example runner."""
...
import pytest
from examples import verify_tmp_monthly_report_noop as example
...
def _result(status: str = "verified_noop") -> TmpNoopVerificationResult:
    return TmpNoopVerificationResult(
        ...
        operation_name="TMP_APPV2_OP_MONTHLY_REPORT_MGR_SET",
        preflight_name="TMP_APPV2_OP_MONTHLY_REPORT_MGR_GET",
    )


def test_monthly_report_example_rejects_before_credentials_or_client() -> None:
    with (
...
def test_monthly_report_example_writes_owner_only_value_free_result(
    ...
    assert evidence["operation_hex_code"] == "0x4223"
    assert evidence["parameter_values_retained"] is False
    assert output.stat().st_mode & 0o777 == 0o600


def test_monthly_report_example_persists_nonverified_result_before_raising(
    monkeypatch: pytest.MonkeyPatch,
    tmp_path: Path,
) -> None:
    monkeypatch.setenv("DECO_PASSWORD", "secret")
